...
class SFConnection {
    ...
    private readonly logEntryRegex: RegExp = /\d+:\d+:\d+\.\d+ \(\d+\)\|CODE_UNIT_STARTED\|\[EXTERNAL\]\|\w+\|(.+)/;
    ...
    public async setProjectPath(path: string): Promise<boolean> {
        try {
            ...
            logger.error(`Error setting project path: ${error}`);
            return false;
        }
    }
    ...
    public listenForMinimalLogInfo(app: Express): void {
        app.get("/log/:logid", async (req: Request, res: Response) => {
            ...
            try {
                ...
                logger.error(`Error fetching log body: ${error}`);
